...
TEST(IsSubRangeTest, SameNonEmptyString) {
  const verible::document_view substring = "nonempty";
  EXPECT_TRUE(IsSubRange(substring, substring));
}
...
TEST(IsSubRangeTest, MixedStringViewStdString) {
  ...
  const verible::document_view substring = verible::document_view(superstring).substr(1, 3);
  // Note: std::string and verible::document_view iterators are not directly
  // comparable to each other.
  EXPECT_TRUE(IsSubRange(substring, verible::document_view(superstring)));
}
...
TEST(IsSubRangeTest, StringAndIterator) {
  ...
  const auto substring =
      iterator_range(superstring.begin() + 1, superstring.begin() + 3);
...
TEST(IsSubRangeTest, DifferentStringViews) {
  const document_view a = "twiddle-dee";
  const document_view b = "twiddle-dum";
...
TEST(IsSubRangeTest, SubStringView) {
  const document_view superstring = "not-empty";
...
TEST(IsSubRangeTest, SuperStringView) {
  const document_view superstring = "also-nonempty";
...
TEST(IsSubRangeTest, DerivedSubStringView) {
  const document_view str = "qwertyuiop";
...
TEST(BoundsEqualTest, DifferentStringViews) {
  const verible::document_view a = "twiddle-dee";
  const verible::document_view b = "twiddle-dum";
...
TEST(BoundsEqualTest, SubStringView) {
  const verible::document_view superstring = "not-empty";
...
TEST(BoundsEqualTest, SuperStringView) {
  const verible::document_view superstring = "also-nonempty";
...
TEST(BoundsEqualTest, DerivedSubStringView) {
  const verible::document_view str = "qwertyuiop";
...
TEST(SubRangeIndicesTest, Empty) {
  ...
  const auto supersequence = iterator_range(v.begin(), v.end());
  ...
  for (size_t i = 0; i < supersequence.size(); ++i) {
    for (size_t j = i; j < supersequence.size(); ++j) {
      const auto subsequence =
          iterator_range(supersequence.begin() + i, supersequence.begin() + j);
