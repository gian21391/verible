...
class BlankLineRule : public LineLintRule {
 public:
  ...
  void HandleLine(document_view line) final {
    if (line.empty()) {
...
TEST(LineLinterTest, OneRuleAcceptsLines) {
  std::vector<document_view> lines{"abc", "def"};
  LineLinter linter;
...
class EmptyFileRule : public LineLintRule {
 public:
  ...
  void HandleLine(document_view line) final { ++lines_; }
...
TEST(LineLinterTest, FinalizeAccepts) {
  std::vector<document_view> lines{"x"};
  LineLinter linter;
