...
static void ForAllSubstringRanges(
    document_view sv, const std::function<void(document_view)> &func) {
  for (auto start_iter = sv.begin(); start_iter != sv.end(); ++start_iter) {
    for (auto end_iter = start_iter; end_iter != sv.end(); ++end_iter) {
      ...
      func(make_document_view_range(start_iter, end_iter));
    }
  }
}
...
TEST(StringViewSuperRangeMapTest, SuperRangeNotInSet) {
  ...
  constexpr document_view text("text");
  svmap.must_emplace(text);
  EXPECT_DEATH(svmap.must_find(document_view("never-there")), "");
}

TEST(StringViewSuperRangeMapTest, TwoStrings) {
  StringViewSuperRangeMap svmap;
  constexpr document_view text1("hello"), text2("world");
  {
    const auto new_iter = svmap.must_emplace(text1);
    EXPECT_NE(new_iter, svmap.end());
    EXPECT_FALSE(svmap.empty());
    EXPECT_TRUE(BoundsEqual(
        make_document_view_range(new_iter->first, new_iter->second), text1));
  ...
  }
  {
    ...
    EXPECT_FALSE(svmap.empty());
    EXPECT_TRUE(BoundsEqual(
        make_document_view_range(new_iter->first, new_iter->second), text2));
    EXPECT_TRUE(BoundsEqual(svmap.must_find(text2), text2));
  }

  ForAllSubstringRanges(text1, [&svmap, text1](document_view subrange) {
    EXPECT_TRUE(BoundsEqual(svmap.must_find(subrange), text1));
  });
  ForAllSubstringRanges(text2, [&svmap, text2](document_view subrange) {
    EXPECT_TRUE(BoundsEqual(svmap.must_find(subrange), text2));
  });
}

TEST(StringViewSuperRangeMapTest, EraseString) {
  constexpr document_view text1("onestring");
  constexpr document_view text2("another");
  StringViewSuperRangeMap svmap;
...
static document_view DocumentViewKey(
    const std::unique_ptr<const std::string> &owned) {
  return document_view(*ABSL_DIE_IF_NULL(owned));
}

using StringSet =
    StringMemoryMap<std::unique_ptr<const std::string>, DocumentViewKey>;
...
static document_view InsertStringCopy(StringSet *sset, const char *text) {
  const auto new_iter =
      sset->insert(std::make_unique<std::string>(text));  // copy
  return make_document_view_range(new_iter->first.first, new_iter->first.second);
}

TEST(StringMemoryMapTest, OneElement) {
  StringSet sset;
  const document_view sv(InsertStringCopy(&sset, "OWNED"));

  // Check all valid substring ranges
  ForAllSubstringRanges(sv, [&sset, sv](document_view subrange) {
    const auto *f = sset.find(subrange);
    ASSERT_NE(f, nullptr) << "subrange returned nullptr: " << subrange;
    const document_view fv(**f);
...
TEST(StringMemoryMapTest, MultipleElements) {
  ...
  const document_view sv1(InsertStringCopy(&sset, "AAA"));
  const document_view sv2(InsertStringCopy(&sset, "BBBB"));
  const document_view sv3(InsertStringCopy(&sset, "CCCCC"));

  // Check all valid substring ranges
  ForAllSubstringRanges(sv1, [&sset, sv1](document_view subrange) {
    const auto *f = sset.find(subrange);
    ASSERT_NE(f, nullptr) << "subrange returned nullptr: " << subrange;
    const document_view fv(*ABSL_DIE_IF_NULL(*f));
    EXPECT_TRUE(BoundsEqual(fv, sv1)) << "got: " << fv << " vs. " << sv1;
    EXPECT_EQ(fv, "AAA");
  });
  ForAllSubstringRanges(sv2, [&sset, sv2](document_view subrange) {
    const auto *f = sset.find(subrange);
    ASSERT_NE(f, nullptr) << "subrange returned nullptr: " << subrange;
    const document_view fv(*ABSL_DIE_IF_NULL(*f));
  ...
  ForAllSubstringRanges(sv3, [&sset, sv3](document_view subrange) {
    const auto *f = sset.find(subrange);
    ASSERT_NE(f, nullptr) << "subrange returned nullptr: " << subrange;
    const document_view fv(*ABSL_DIE_IF_NULL(*f));
