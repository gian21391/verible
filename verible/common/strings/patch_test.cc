...
TEST(MarkedLineParseTest, InvalidInputs) {
  constexpr document_view kTestCases[] = {
      "", "x", "x213", "abc", "diff", "====",
  };
...
TEST(HunkHeaderParseTest, InvalidInputs) {
  ...
  constexpr document_view kValidText = "@@ -4,8 +5,6 @@";

  for (size_t i = 0; i < kValidText.length(); ++i) {
    std::string deleted(kValidText.to_string());
    deleted.erase(i);
    HunkHeader h;
  ...
  for (size_t i = 0; i < kValidText.length(); ++i) {
    document_view deleted(kValidText.substr(0, i));
    HunkHeader h;
    EXPECT_FALSE(h.Parse(deleted).ok()) << " input: \"" << deleted << '"';
  }

  for (size_t i = 1; i < kValidText.length(); ++i) {
    document_view deleted(kValidText.substr(i));
    HunkHeader h;
    EXPECT_FALSE(h.Parse(deleted).ok()) << " input: \"" << deleted << '"';
  }
}
...
TEST(HunkHeaderParseTest, MalformedOldRange) {
  constexpr document_view kInvalidText = "@@ 4,8 +5,6 @@";
  HunkHeader h;
...
TEST(HunkHeaderParseAndPrintTest, ValidInput) {
  constexpr document_view kValidText = "@@ -14,8 +5,16 @@";
  HunkHeader h;
...
TEST(HunkHeaderParseAndPrintTest, ValidInputWithContext) {
  constexpr document_view kValidText("@@ -4,28 +51,6 @@ void foo::bar() {");
  HunkHeader h;
  EXPECT_TRUE(h.Parse(kValidText).ok()) << " input: \"" << kValidText << '"';
...
TEST(SourceInfoParseAndPrintTest, ValidInputsPathOnly) {
  constexpr document_view kPaths[] = {
      "a.txt",
      "p/q/a.txt",
      "/p/q/a.txt",
  };
...
TEST(HunkParseTest, InvalidInputs) {
  const std::vector<document_view> kTestCases[] = {
      // malformed headers:
      {"@@ -1,0 +2,0 @"},
      {"@ -1,0 +2,0 @@"},
      {"@@ -1,0+2,0 @@"},
...
struct UpdateHeaderTestCase {
  document_view fixed_header;
  std::vector<document_view> payload;
};

TEST(HunkUpdateHeaderTest, Various) {
  constexpr document_view kNonsenseHeader = "@@ -222,999 +333,999 @@";
  ...
  for (const auto &test : kTestCases) {
    std::vector<document_view> lines;
...
struct AddedLinesTestCase {
  std::vector<document_view> hunk_text;
...
TEST(HunkVerifyAgainstOriginalLinesTest, LineNumberOutOfBounds) {
  const std::vector<document_view> kHunkText = {
      {
  ...
  const std::vector<document_view> kOriginal = {
      "line1", "line2", "line3",
      // no line4
  };
  Hunk hunk;
...
TEST(FilePatchIsNewFileTest, NewFile) {
  const std::vector<document_view> kInput = {
      "--- /dev/null\t2020-03-30",
      "+++ /path/to/file.txt\t2020-03-30",
      "@@ -0,0 +1,2 @@",
      "+new content 1",
...
class FilePatchPickApplyTest : public FilePatch, public ::testing::Test {
 protected:
  absl::Status ParseLines(const std::vector<document_view> &lines) {
    const LineRange range(lines.begin(), lines.end());
    return Parse(range);
  }
...
struct ReadStringFileSequence : public StringFileSequence {
  ...
  absl::StatusOr<std::string> operator()(std::string_view filename) {
    ...
    ++index_;
    return std::string{file.contents.to_string()};
  }
};
...
struct ExpectStringFileSequence : public StringFileSequence {
  ...
  absl::Status operator()(std::string_view filename, document_view src) {
    // ASSERT_LT(i, files_.size());  // can't use ASSERT_* which returns void
    if (index_ >= files_.size()) {
      return absl::OutOfRangeError(
...
TEST_F(FilePatchPickApplyTest, EmptyPatchNoPrompt) {
  ...
  constexpr document_view kOriginal = "aaa\nbbb\nccc\n";
  constexpr document_view kExpected = kOriginal;  // no change
...
TEST_F(FilePatchPickApplyTest, ErrorWritingFileInPlace) {
  ...
  auto error_file_writer = [=](std::string_view, document_view) {
...
TEST_F(FilePatchPickApplyTest, OneHunkNotApplied) {
  ...
  constexpr document_view kOriginal =
      "aaa\n"
      "bbb\n"
      "ccc\n"
      "ddd\n"
      "eee\n";
  constexpr document_view kExpected = kOriginal;
...
TEST_F(FilePatchPickApplyTest, PatchInconsistentWithOriginalText) {
  ...
  constexpr document_view kOriginal =
      ...
      "ddd\n"
      "eee\n";
  constexpr document_view kExpected =
      "aaa\n"
      "bbb\n"
      "ddd\n"
      "eee\n";
...
TEST_F(FilePatchPickApplyTest, RejectAllDeletions) {
  ...
  constexpr document_view kOriginal =
      ...
      "ggg\n"
      "hhh\n";
  constexpr document_view kExpected = kOriginal;  // no changes
...
TEST(PatchSetAddedLinesMapTest, NewAndExistingFile) {
  constexpr document_view patch_contents =  //
