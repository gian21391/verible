...
static constexpr document_view kTestToken[] = {
    "test_token1",
    "test_token2",
    "test_token3",
    "test_token4",
...
TEST(StringSpanOfSymbolTest, LeafOnlyNonemptyText) {
  constexpr document_view text("asdfg");
  SymbolPtr symbol = Leaf(1, text);
  const auto range = StringSpanOfSymbol(*symbol);
  EXPECT_TRUE(BoundsEqual(range, text));
}
...
TEST(StringSpanOfSymbolTest, TwoLeavesOneTree) {
  constexpr document_view text("aaabbb");
  SymbolPtr symbol =
...
TEST(TreePrintTest, RawPrint) {
  constexpr document_view text("leaf 1 leaf 2 leaf 3 leaf 4");
  ...
  constexpr document_view expected =
      "Node @0 {\n"
      "  Leaf @0 (#0: \"leaf 1\")\n"
      "  Node @1 {\n"
      "    Leaf @0 (#1: \"leaf 2\")\n"
...
const document_view kFindSubtreeTestText("abcdef");
const document_view kFindSubtreeTestSubstring(
...
constexpr document_view kBaseTextPadded("_abcdefghijklmnopqrst_");
constexpr document_view kBaseText = {kBaseTextPadded.data() + 1,
...
TEST(PruneSyntaxTreeAfterOffsetTest, RootLeafNeverRemoved) {
  constexpr document_view text("baz");
...
TEST(PruneSyntaxTreeAfterOffsetTest, PruneTrailingNullNodes) {
  constexpr document_view text("foo bar");
  const document_view foo(text.substr(0, 3)), bar(text.substr(4, 3));
...
TEST(PruneSyntaxTreeAfterOffsetTest, PruneTrailingNullNodesRecursive) {
  constexpr document_view text("foo bar BQ");
  const document_view foo(text.substr(0, 3)), bar(text.substr(4, 3)),
      bq(text.substr(8, 2));
  SymbolPtr tree = TNode(0,                   // noformat
                         nullptr,             // noformat
...
TEST(TrimSyntaxTreeTest, OneLeafNotTrimmed) {
  constexpr document_view text("ddddddddddddddd");
  const document_view token(text.substr(5, 5));
...
TEST(TrimSyntaxTreeTest, OutOfRangeRight) {
  constexpr document_view text("dddddddddddddddddddddddddd");
  SymbolPtr tree = FakeSyntaxTree();
