...
static TokenInfo Token(document_view s) { return TokenInfo(NOT_EOF, s); }
...
TEST(ParserVerifierTest, PartialUnmatched) {
  constexpr document_view foo("foo");
  constexpr document_view bar("bar");
  auto root = Node(Leaf(NOT_EOF, foo));
  TokenSequence stream = {Token(foo), Token(bar)};
...
TEST(ParserVerifierTest, SeveralPartialUnmatched) {
  constexpr document_view foo("foo");
  constexpr document_view bar1("bar1");
  constexpr document_view bar2("bar2");
  constexpr document_view mee("mee");
