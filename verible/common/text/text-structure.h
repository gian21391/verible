...
class TextStructureView {
 public:
  ...
  explicit TextStructureView(document_view contents);
  ...
  document_view Contents() const { return contents_; }
  ...
  const LineColumnMap &GetLineColumnMap() const {
    return *ABSL_DIE_IF_NULL(lazy_lines_info_.Get(contents_).line_column_map);
  }
  ...
  LineColumnRange GetRangeForText(document_view text) const;

  // checks if a given text belongs to the TextStructure
  bool ContainsText(document_view text) const;
  ...
  // to a new owner.
  void RebaseTokensToSuperstring(document_view superstring,
                                 document_view src_base, int offset);
 ...
 protected:
  ...
  struct LinesInfo {
    ...
    const LinesInfo &Get(document_view contents);
  };
...
class TextStructure {
 private:
  ...
  explicit TextStructure(document_view contents);
